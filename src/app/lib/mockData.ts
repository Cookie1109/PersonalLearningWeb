...
export const mockQuizQuestions: QuizQuestion[] = [
  {
    ...
    options: [
      'Không có sự khác biệt, cả hai đều làm cùng một việc',
      '`.loc[]` truy cập theo nhãn (label), `.iloc[]` truy cập theo vị trí số nguyên',
      '`.iloc[]` truy cập theo nhãn (label), `.loc[]` truy cập theo vị trí số nguyên',
  ...
  },
  {
    ...
    options: ['.describe()', '.summary()', '.info()', '.stats()'],
    correctIndex: 2,
