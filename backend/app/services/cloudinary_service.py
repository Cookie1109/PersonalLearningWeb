...
def upload_avatar_image(
    *,
    ...
    cloud_name, api_key, api_secret = _require_cloudinary_config()
    settings = get_settings()
    folder = (settings.cloudinary_avatar_folder or "personal-learning/avatars").strip("/")

    original_name = file_name or f"avatar-{uuid4().hex}.png"
    stem = _safe_public_id_segment(Path(original_name).stem)
    public_id = f"user_{user_id}/avatar-{stem}-{uuid4().hex[:10]}"
